...
const EditAddress: React.FC = () => {
    ...
    const loadAddress = async (addressId: string) => {
        try {
            ...
            setZipCode(address.zip_code.toString());
        } catch (error) {
            console.error("Error al cargar la dirección:", error);
            setErrors({ general: "No se pudo cargar la dirección." });
        } finally {
            setLoading(false);
        }
    };
    ...
    const validateFields = () => {
        ...
        if (!number.trim()) newErrors.number = "Este campo es obligatorio.";
        ...
        if (!zip_code.trim()) {
            ...
            newErrors.zip_code = "El código postal debe tener exactamente 5 dígitos.";
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!id) {
            setErrors({ general: "ID de dirección no encontrado." });
            return;
        }
        ...
        try {
            await addressService.update(updatedAddress as Address);
            setSuccess(true);
        } catch (error) {
            console.error("Error al actualizar dirección:", error);
            setErrors({ general: "No se pudo actualizar la dirección." });
        }
    };
    ...
    if (loading) {
        return (
            <div className="max-w-md mx-auto px-4 py-8">
                <p className="text-center">Cargando dirección...</p>
            </div>
        );
    }
